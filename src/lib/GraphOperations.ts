...
class GraphOperations extends EventEmitter {
  ...
  constructor(
    ...
    { showProgBar, concurrency }: GraphOperationsOpts = {},
  ) {
    super();
  ...
  async rightRWStep(
    ...
    const q = new Query()
      .select(o)
      .where(
        Q(s as Quad_Subject, p as Quad_Predicate, V(o)),
        BIND(RAND(), "sortKey"),
      )
      .orderBy("sortKey")
      .limit(1);
  ...
  async _randomWalk(
    ...
    node: Term,
    len: number,
  ): Promise<{ status: string[]; nodes: Term[] }> {
    ...
    const pathIsLoop = (path: Term[]) =>
      path.length > 1 && path[0].value === path[path.length - 1].value;
    ...
    while (
      ...
      if (
        !rightFinished &&
        !pathIsLoop(path) &&
        rightNode?.termType === "NamedNode"
      ) {
        const x: Term | undefined = await this.rightRWStep(
          rightNode as Quad_Subject,
      ...
      if (!leftFinished && path.length < len && !pathIsLoop(path)) {
        const x: Term | undefined = await this.leftRWStep(
          "x",
          pred as Quad_Predicate,
          leftNode as Quad_Object,
        );
        const _s = this.isRandomWalkOver(x, visitedNodes);
        if (x) {
          visitedNodes.add(x.value);
  ...
  async calcRandomWalks(
    preds: { [key: string]: BasePredicate },
    pctg = 1,
    walkLength = 5,
  ): Promise<{ [key: string]: PredicateWalks }> {
  ...
  async calcSubjectCoverage(
    ...
    return Object.fromEntries(
      cov.map((c) => [
        c.get("p")?.value as string,
        Number(c.get("cov")?.value),
      ]),
    );
  }
  ...
  async globalMetrics(seedQuery: Query): Promise<GlobalMetrics> {
    ...
    const totalNodes = await this._runQuery(
      new Query()
        .select(COUNT("x", "total", "distinct"))
        .where(Q(V("x"), V("p"), V("o")), UNION, Q(V("s"), V("p"), V("x"))),
    );

    const totalSubjects = await this._runQuery(
      new Query()
        .select(COUNT("x", "total", "distinct"))
        .where(Q(V("x"), V("p"), V("o"))),
    );
