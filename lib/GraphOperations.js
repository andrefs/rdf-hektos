...
class GraphOperations extends EventEmitter {
  ...
  constructor(repo, {host='http://localhost', port='7201', showProgBar}){
    super();
    if(showProgBar){
      this._handleEvents();
    }
    this._endpointUrl = `${host}:${port}/repositories/${repo}`;
  }

  async init(){
    this._store = new Store({endpointUrl: this._endpointUrl});
  }
